...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  tx = await execute("MyDanDefi", { from: deployer, gasLimit: 1000000 }, "setDurations", durations, bonusRates);
  ...
  tx = await execute("MyDanDefi", { from: deployer, gasLimit: 1000000 }, "insertMembershipTiers", tiers);
  ...
  tx = await execute("MyDanDefi", { from: deployer, gasLimit: 1000000 }, "setAssetsUnderManagementCap", assetUnderManagementCap);
  console.log(`setAssetsUnderManagementCap ${tx.transactionHash}`);
  let referralBonusRates = [0, 600, 200, 200, 100, 100, 100, 100];
  tx = await execute("MyDanDefi", { from: deployer, gasLimit: 1000000 }, "setReferralBonusRates", referralBonusRates);
